...
export default function HeroMessmer() {
    ...
    const sections = [
        "Nas terras onde a graça jamais tocou…",
        "Existe um nome que foi apagado da história…",
        "Messmer.",
        "O Portador da Chama Proibida.",
        "Sua chama não ilumina…\nEla consome.",
        "Onde ele passa…\nSó restam cinzas… e silêncio.",
        "Apagado não por fraqueza…\nMas porque até os deuses temem seu nome.",
        "E mesmo esquecido…\nEle nunca partiu.",
        "O fogo ainda vive.\nEsperando…\nPor você."
    ]
    ...
    // Adicione esse useEffect para trocar o texto a cada 3s
    useEffect(() => {
        if (!mounted) return
    ...
    return (
        <div
            ...
            className="relative h-[100vh] max-h-[600px] w-full overflow-hidden"
            style={{ cursor: 'none' }}
        >
            ...
            {/* Texto fixo */}
            <div className="absolute inset-0 z-40 flex-col items-center justify-center px-6 pointer-events-none">
                <h1
                    className="text-red-rune whitespace-pre-line text-center text-3xl md:text-4xl lg:text-8xl"
                    style={{
                ...
                <h2 className='text-red-rune goth-text whitespace-pre-line tracking-tight text-center text-3xl md:text-2xl lg:text-2xl drop-shadow-[0_0_20px_rgba(0,0,0,0.8)]'>O Portador da Chama Proibida.</h2>
            </div>



            {/* Vinheta âmbar nas bordas
            <div
                className="absolute inset-0 z-20 pointer-events-none"
                style={{
                    boxShadow: `
                        inset 0 0 30px 10px rgba(0,0,0,0.7),
                        inset 0 -60px 80px rgba(120, 40, 0, 0.4),
